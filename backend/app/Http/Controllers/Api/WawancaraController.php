...
class WawancaraController extends Controller
{
    ...
    public function selesai(Request $request, AssessmentService $service, $id)
    {
        ...
        if ($data['rekomendasi'] === 'lulus') {
            ...
            Sertifikat::firstOrCreate(['user_id' => $peserta->user_id, 'asesmen_id' => $peserta->asesmen_id], ['nomor_sertifikat' => 'SKW-'.now()->format('Ymd').'-'.\Illuminate\Support\Str::upper(\Illuminate\Support\Str::random(6)), 'kompetensi_id' => $peserta->asesmen?->kompetensi_id, 'level_id' => $peserta->asesmen?->level_id, 'nilai_akhir' => $peserta->nilai, 'kategori_kompetensi' => $service->kategori((float) $peserta->nilai), 'tanggal_terbit' => now(), 'tanggal_expired' => now()->addYears(3), 'is_active' => true]);
