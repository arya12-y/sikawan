...
function DataTable({ fields, rows, onEdit, onDelete, canEdit, canDelete }) {
  return <div className="overflow-hidden rounded-xl border border-[#1E1E2E]"><table className="w-full text-left text-sm"><thead className="bg-[#09090E] text-xs uppercase tracking-wider text-slate-400"><tr>{fields.map((f) => <th className="px-[18px] py-3.5 text-center font-semibold" key={f.name}>{f.label}</th>)}<th className="px-[18px] py-3.5 text-center font-semibold">Aksi</th></tr></thead><tbody className="divide-y divide-slate-100 bg-[#14141E]">{rows.map((row) => <tr className="transition hover:bg-[#14141E]/[0.03]/80" key={row.id}>{fields.map((f) => <td className="px-[18px] py-4 text-center text-slate-300" key={f.name}>{String(row[f.name] ?? '')}</td>)}<td className="whitespace-nowrap px-[18px] py-4 text-center"><div className="flex items-center justify-center gap-2">{canEdit && <button onClick={() => onEdit(row)} className="inline-flex items-center justify-center rounded-xl border border-[#262636] p-2 text-sm text-slate-400 transition-colors hover:bg-[#1A1A26] hover:text-slate-200" title="Edit"><Pencil className="h-4 w-4" /></button>}{canDelete && <button onClick={() => onDelete(row)} className="inline-flex items-center justify-center rounded-xl border border-rose-600/20 p-2 text-sm text-rose-400 transition-colors hover:bg-rose-500/10" title="Hapus"><Trash2 className="h-4 w-4" /></button>}</div></td></tr>)}</tbody></table></div>
}
...
function GenericMasterPage({ endpoint, fields, title, filters = [] }) {
  ...
  const save = async (data) => { setSaving(true); try { if (current?.id) await api.put(`${endpoint}/${current.id}`, data); else await api.post(endpoint, data); await load(); toast('success', `Berhasil menyimpan ${title}`); setCurrent(null); setShowForm(false) } catch (error) { const validationErrors = error.response?.data?.errors; const message = validationErrors ? Object.values(validationErrors).flat().join('; ') : error.response?.data?.message || `Gagal menyimpan ${title}`; toast('error', `Gagal: ${message}`) } finally { setSaving(false) } }
